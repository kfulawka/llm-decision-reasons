...
base_folders = list.files('02_llm_analyses/02_reports_raw_dat',
                          full.names = T)
names(base_folders) = list.files('02_llm_analyses/02_reports_raw_dat',
                                 full.names = F)

base_res_path = '02_llm_analyses/02_reports_extracted/'

# LOOP OVER LLMS ----------------------------------------------------------

for(llm in names(base_folders)) {
  
  # extract confidence scores -----------------------------------------------
  
  # Run and combine all responses
  df_all = process_files_long_format(base_folders[llm])
  ...
  df_all$assessment = suppressWarnings(
    as.numeric(
      ...
      sub(".*final_assessment_score.*:\\s*([0-9\\.]+).*", "\\1", df_all$llm_output, ignore.case = TRUE)
    )
  )
  ...
  if(nrow(missing_entries) > 0) {
    ...
    # save for inspection
    saveRDS(missing_entries, paste0(base_res_path, 'reanalysis_', llm, '.rds'))
    # 
  ...
  # save
  saveRDS(dd, paste0(base_res_path, 'conf_choice_dat_long_', llm, '.rds'))
  ...
  saveRDS(dw, paste0(base_res_path, 'conf_choice_dat_wide_', llm, '.rds'))
