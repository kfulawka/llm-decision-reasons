...
res_folder = '02_llm_analyses/01_understanding_raw_dat'
res_files = list.files(res_folder)
#
res_files = res_files[res_files != 'extracted']
...
extract_llm_dec = function(x) {
  ...
  xx = sapply(1:20, function(ii) {
    ...
    #
    y <- gsub("[\\*\\_\"]", "", y)  # remove asterisks, underscores, etc 
    y <- gsub("FINAL ANSWER", "finalanswer", y) # to capture GPT-4 output
    
    #
    pattern <- "finalanswer\\*?\\*?:?\\s*(A|B|INDIFFERENT)"
...
for(mod_res in res_files) {
  
  # data --------------------------------------------------------------------
  
  #
  llm_r_csvs = list.files(full.names = T,
                          path = paste0(res_folder, '/', mod_res)
  )
  
  llm_r_raw = lapply(llm_r_csvs, function(xx) {
    
    # get reason name
    rn = gsub(paste0(res_folder, '/', mod_res, "/|\\.csv"), '', xx)
    rn = gsub(' ', '_', rn)
    # read in the results
    x = read.csv(xx)
    # remove index column
  ...
  # into rds
  saveRDS(dr_llm, paste0('02_llm_analyses/01_undr_extracted/', mod_res, '_co.rds'))
  ...
  saveRDS(d_comp, paste0('02_llm_analyses/01_undr_extracted/', mod_res, '_eval.rds'))
